...
class WinForm(QWidget):
	...
	def pBtn_Click_Function(self,pBtn_Object_Num):
		...
		if self.Displaytext.toPlainText() == WinForm.Calculate_Char_SHOW_SAVE:
			if self.Displaytext.toPlainText() == "":
				...
				List_Save = len(WinForm.Calculate_Char_SHOW_SAVE) - len(WinForm.Calculate_Char_SHOW)
				if List_Save == 0:
					...
					WinForm.Calculate_Char_Operation_List = WinForm.Calculate_Char_Operation_List_SAVE[:(-1*List_Save)]
					WinForm.Calculate_Char_SHOW_List = WinForm.Calculate_Char_SHOW_List_SAVE[:(-1*List_Save)]
			if self.pBtn_TextList_SHOW_TextEdit[pBtn_Object_Num] == "=":
				if WinForm.Calculate_Char_SHOW == WinForm.Calculate_Char_SHOW_SAVE:
					WinForm.Calculate_Char_SHOW_List.append("\n"+str(self.Calculate_Function()))
			...
			if self.Displaytext.toPlainText() == "":
				...
				if List_Save == 0:
					...
					WinForm.Calculate_Char_Operation_List_SAVE.append(WinForm.Calculate_Char_SHOW[List_Save:])
					WinForm.Calculate_Char_Operation_List = WinForm.Calculate_Char_Operation_List_SAVE
					WinForm.Calculate_Char_SHOW_List_SAVE.append(WinForm.Calculate_Char_SHOW[List_Save:])
					WinForm.Calculate_Char_SHOW_List = WinForm.Calculate_Char_SHOW_List_SAVE
			if self.pBtn_TextList_SHOW_TextEdit[pBtn_Object_Num] == "=":
				if WinForm.Calculate_Char_SHOW == WinForm.Calculate_Char_SHOW_SAVE:
					WinForm.Calculate_Char_SHOW_List.append("\n" + str(self.Calculate_Function()))
	...
	def Calculate_Function(self):
		WinForm.Calculate_Char_Expression = self.Displaytext.toPlainText().strip().split("\n").pop()
