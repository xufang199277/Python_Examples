...
class PaintWindow(wx.Window):
    def __init__(self, parent, id):
        ...
        self.SetBackgroundColour("black") #设置画板的背景色
        self.color = "Green"  #画笔的初始颜色
        self.thickness = 8 #设置画笔的初始粗细

        # 创建一个画笔
        self.pen = wx.Pen(self.color, self.thickness, wx.SOLID)  #wx.Pen方法用于创建画笔
        self.lines = []
        self.curLine = []
        self.pos = (0, 0)
        self.InitBuffer()
    ...
    def InitBuffer(self):
        size = self.GetClientSize() #得到客户区域的尺寸

        # 创建缓存的设备上下文
        self.buffer = wx.EmptyBitmap(size.width, size.height) #创建一个与客户区域大小相同的空位图
        dc = wx.BufferedDC(None, self.buffer) #在位图上使用缓存的设备上下文

        # 使用设备上下文
        dc.SetBackground(wx.Brush(self.GetBackgroundColour())) #设置设备上下文（即客户区域）的背景色
        dc.Clear()
        self.DrawLines(dc)
    ...
    def OnLeftDown(self, event):
        self.curLine = []  #每次鼠标左键按下，线条长度清零

        # 获取鼠标位置
        self.pos = event.GetPositionTuple()
        self.CaptureMouse() #可以删除

    def OnLeftUp(self, event):
        if self.HasCapture(): #注释掉后没有任何影响
            self.lines.append((self.color,
                               self.thickness,
                               self.curLine)) #self.curLine可以删除
            # self.curLine = []
            self.ReleaseMouse()
